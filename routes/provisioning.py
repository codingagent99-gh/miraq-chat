"""
routes/provisioning.py — Tenant activation endpoints.
...
(or re-register) a paid tenant with the MiraQ backend.

POST /activate-free — self-serve free-tier activation, unauthenticated by
design for now (see its own docstring for the accepted risk and the
hardening path).

Both are exempt from tenant resolution (see store_registry._EXEMPT_PATHS) —
they create the very tenant row that resolution would otherwise need.
"""
...
def _derive_db_name(seed: str) -> str:
    ...
    Paid path (provision_tenant): seed is license_id.
    Free path (activate_free): seed is str(tenant_id) instead — identity,
    not license — so db_name stays stable across a free→paid upgrade that
    keeps the same tenant_id but swaps license_id. Existing tenants are
    unaffected either way: db_name is stored once at creation and never
    re-derived.
    """
...
def _per_tenant_tables():
    """
    The tables that belong in each tenant's OWN physical database.
    Deliberately NOT db.metadata.tables (i.e. not a blanket db.create_all())
    — Tenant and ShopifyToken are control-plane-only (they live in the main
    database, routed there by _TenantRoutingSession's default bind). A
    blanket create_all() against a tenant's own database would create empty
    `tenants` and `shopify_tokens` tables there too — harmless by itself,
    but it means a stray query that runs after g.db_engine is bound (which
    _TenantRoutingSession routes to the tenant's DB regardless of which
    model it's for) would find an empty table and return "no rows" instead
    of the loud "relation does not exist" that catches the bug immediately.
    """
...
def _create_tenant_schema(db_name: str, base_dsn: str) -> None:
    ...
    with throwaway.app_context():
        _db.metadata.create_all(bind=_db.engine, tables=_per_tenant_tables())
...
def _stamp_alembic_version(tenant_dsn: str, db_name: str) -> None:
    ...
    No-ops (with a warning) if migrations/ has no revisions yet — safe to
    call before that history exists; the tenant's tables are already
    created above regardless.
    """
    import psycopg2
...
def provision_tenant():
    ...
    #      free row was created with tenant_id = this UUID) and a re-activation.
    #      The license attaches in place — DB, snapshot, and the tenant_id-keyed
    #      resident loader all stay put.
    #   2. Else match by the license_id column. Catches a re-activation whose
    #      backfilled tenant_id does NOT equal the plugin UUID — keeps its
    #      existing tenant_id, no duplicate.
...
def activate_free():
    ...
    #   2. gate "active" trust on the first valid catalog-push, if catalog-push
    #      is ever adopted for a tenant whose host WAFs backend-to-WooCommerce
    #      calls (see the conversion plan's D2) — not adopted for anyone yet.
    #   3. optional best-effort nonce callback to wp_base_url (must NOT hard-block:
    #      some hosts block inbound calls entirely)
    ...
    tenant.woo_key = woo_key
    tenant.woo_secret_encrypted = encrypt_secret(woo_secret)
    # license_expires_at stays NULL — free tenants never expire (Tenant.is_active
