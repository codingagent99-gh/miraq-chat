...
class BulkOrderEvaluator(IntentEvaluator):
    ...
    def evaluate(self, text: str, entities: ExtractedEntities) -> Tuple[Optional[Intent], float]:
        ...
        # "2 each of Harmony, Adams" reaches neither check above: the digit
        # sits on the first fragment and there is no order verb anywhere, so
        # Check 1 qualifies nothing and Check 2 never runs. It went to
        # CatalogSearchEvaluator instead and came back as a product search —
        # with the leading "2" read as a 2cm thickness filter.
        #
