...
def classify(utterance: str) -> ClassifiedResult:
    ...
    _question_word_re = r'\b(?:what|which|how\s+many|tell\s+me\s+(?:the|about))\s+(?:[\w\']+\s+){0,3}'
    _qmask_loader = get_store_loader()
    if _qmask_loader and _qmask_loader.all_attributes_raw:
        _sorted_attrs = sorted(
            _qmask_loader.all_attributes_raw,
            key=lambda a: len(a.get("attribute_label", "")),
            reverse=True
        )
        _words_to_mask = set()
        for _qattr in _sorted_attrs:
            ...
            for _qword in ([_qlabel] + _qlabel.split()):
                if len(_qword) < 4:
                    continue
                if re.search(_question_word_re + re.escape(_qword) + r's?\b', text, re.IGNORECASE):
                    _words_to_mask.add(re.escape(_qword) + r's?')
                    if _qlabel not in entities.target_attributes:
                        entities.target_attributes.append(_qlabel)
                    break
        for _w in _words_to_mask:
            attr_text = re.sub(_w, ' ', attr_text, flags=re.IGNORECASE)
