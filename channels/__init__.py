...
The chat pipeline is untouched: routes/channel.py runs the normal /chat turn
and hands the widget-shaped JSON to a renderer here, which turns it into the
channel's native Send-API message objects. The calling service only has to
POST each object, in order, to Meta (or its BSP).
...
__all__ = ["RENDERERS", "SUPPORTED_CHANNELS", "decode_reply_id"]